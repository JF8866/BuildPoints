...
object ColorTemplate {
    ...
    private fun color2String(colorGroup: String, index: Int, color: Int): String {
        return """<color name="${colorGroup.toLowerCase()}_${index}">#${String.format("%08X", color)}</color>"""
    }

    private fun colorGroup2String(colorGroup: String, colors: IntArray, res: (String) -> Unit) {
        for (i in colors.indices) {
            res(color2String(colorGroup, i, colors[i]))
        }
    }

    fun printColors() {
        val sb = StringBuilder()
        colorGroup2String("LIBERTY", LIBERTY_COLORS, sb::appendLine)
        colorGroup2String("JOYFUL", JOYFUL_COLORS, sb::appendLine)
        colorGroup2String("PASTEL", PASTEL_COLORS, sb::appendLine)
        colorGroup2String("COLORFUL", COLORFUL_COLORS, sb::appendLine)
        colorGroup2String("VORDIPLOM", VORDIPLOM_COLORS, sb::appendLine)
        colorGroup2String("MATERIAL", MATERIAL_COLORS, sb::appendLine)
        println(sb)
    }
